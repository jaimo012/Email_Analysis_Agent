...
function processNewEmails() {
  ...
  // -label:PROCESSED 조건을 제거하여, 새 답장이 온 기존 스레드도 검색 대상에 포함
  ...
  threads.forEach(thread => {
    ...
    // 스레드의 '가장 마지막' 메시지를 가져와서 현재 상태를 파악
    ...
    // [상태 리셋] 이미 라벨이 붙어있던 스레드에 새 메시지가 온 것이라면 라벨을 떼고 재분류
    ...
    if (isGroupMailPending) {
      ...
      // ⭐️ [변경] 분류 작업은 가장 저렴하고 빠른 Gemini 2.5 Flash를 사용합니다.
      const emailCategory = classifyEmailWithGemini(msg);
      ...
      if (emailCategory === 'SPAM') {
        ...
        // ⭐️ 초안 작성은 똑똑한 Claude Sonnet 4.5 사용
